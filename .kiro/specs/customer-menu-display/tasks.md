...
- [~] 1. Tambah kelas CSS baru untuk grid dan kartu menu di `style.css`
...
- [~] 2. Implementasi fungsi-fungsi render baru di `site.js`
  - [~] 2.1 Implementasi `renderEmptyState(message)`
    - Kembalikan HTML string dengan elemen ikon Bootstrap Icons dan teks pesan
    - Gunakan kelas `.menu-empty-state`
    - _Requirements: 4.1, 4.2, 4.4_

  - [~] 2.2 Tulis property test untuk `renderEmptyState`
    - **Property 9: Empty state selalu mengandung pesan yang diberikan**
    - **Validates: Requirements 4.4**

  - [~] 2.3 Implementasi `formatRupiah(harga)` (refactor dari fungsi yang sudah ada)
  ...
  - [~] 2.4 Tulis property test untuk `formatRupiah`
    - **Property 4: Format Rupiah selalu konsisten**
    - **Validates: Requirements 2.3**

  - [~] 2.5 Implementasi `renderMenuCard(menu, isTomorrow)`
  ...
  - [~] 2.6 Tulis property test untuk `renderMenuCard`
  ...
  - [ ] 2.10 Tulis property test untuk `renderFilterBar`
...
- [ ] 3. Checkpoint — Pastikan semua fungsi render baru menghasilkan HTML yang valid
  - Pastikan semua tests pass, tanyakan ke user jika ada pertanyaan.

- [ ] 4. Implementasi `renderMenuSection` dan sambungkan ke `renderPublicPage` di `site.js`
  - [ ] 4.1 Implementasi `renderMenuSection(selector, menus, options)`
  ...
  - [ ] 4.2 Tulis property test untuk `renderMenuSection`
  ...
  - [ ] 4.3 Ganti pemanggilan `renderMenuList` dengan `renderMenuSection` di `renderPublicPage`
...
- [ ] 5. Update struktur HTML section menu di `index.html`
...
- [ ] 6. Checkpoint akhir — Verifikasi integrasi end-to-end
  - Pastikan semua tests pass, tanyakan ke user jika ada pertanyaan.
